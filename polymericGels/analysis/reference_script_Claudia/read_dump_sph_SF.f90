...
real(8) :: dq0,bin0,sfsum1,sfsum2,qr(3),rq,rq0,ki,smax,qmax0,times
...
do iq=-qmax,qmax
	...
	do jq=-qmax,qmax
		...
		do kq=-qmax,qmax			
			...
			sfhis(sbin,1)=sfhis(sbin,1)+(sfsum1*sfsum1+sfsum2*sfsum2)/(ntotav*1.0)
			sfhis(sbin,2)=sfhis(sbin,2)+1			
		end do
	end do
end do
...
filename=trim(filedump0)//trim("/sf_slow.dat")
