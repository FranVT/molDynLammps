...
file_paths=[joinpath(SAVE_DIR,file_names) for it in file_names];
...
fig=Figure()
    ...
    for it_sys in 1:5
        q=(2*pi).*sqrt.(Sq[it_sys].mag)./first(data_bySystem[it_sys].L)
        lambda=2*pi./q
        scatterlines!(ax,q,Sq[it_sys].Sq10000000,
               label=latexstring(100*first(data_bySystem[it_sys].phi))
              )
    end
