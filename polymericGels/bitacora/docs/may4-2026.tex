...
en donde,
\begin{gather*}
    A^2 = \sum_{j=1}^{N}\cos\qty(\vec{q}\cdot\vec{r}_{j}),\quad
    B^2 = \sum_{j=1}^{N}\sin\qty(\vec{q}\cdot\vec{r}_{j}).
\end{gather*}
Considerando que \(\vec{q} = \frac{2\pi}{\lambda}\hat{r}\) con \(\hat{r}\) siendo el vector de posición unitario en coordenadas esféricas.
El producto punto lo calculo de la siguiente manera,
\begin{align*}
    \vec{q}\cdot\vec{r}_{j}&=\frac{2\pi}{\lambda}\hat{r}\cdot\vec{r}_{j} \\
                           &=\frac{2\pi}{\lambda}\left(r_{1}r_{jx} + r_{2}r_{jy} + r_{3}r_{jz} \right)
\end{align*}
